...
<div id="pagina_sistema">
		...
		<?php }?>	
		 <?php if ($_SESSION['con_baj'] == 3){?> 
					<h2> <img src="img/back_64x64.png" border="0" alt="Modulo" align="absmiddle">CAMBIAR FECHA A ASIENTO CONTABLE </h2>
                      <hr style="border:1px dashed #767676;">
                       <div id="error_login"> 
         				 <img src="img/checkmark_32x32.png" align="absmiddle">
           					Registre la nueva fecha del asiento contable
        			   </div>	
		<?php }?>			   
        	<?php } elseif($_SESSION['menu'] == 10){?>
			  ...
			  <strong>
                 <?php //$nitcli = $_SESSION['cli_nit'];
		        // $_SESSION['cli_nom'] = leer_empresa($nitcli);
				// $plan_nit = $_SESSION['plan_nit'];
				// echo $plan_nit;
				 ?> 
				 </strong> 
                      <hr style="border:1px dashed #767676;">
...
$_SESSION['nro_doc'] = $nro_doc;
  // if( $_SESSION['cargo'] <> 4){
	...
	/*}else{
	 $con_cab  = "Select CONTA_CAB_GLOSA
	              From contab_cabec_cli where CONTA_CAB_EMP = $nitcli and CONTA_CAB_NRO = $nro_doc  
				  and CONTA_CAB_USR_BAJA is null";
 ...
 //if( $_SESSION['cargo'] <> 4){	
	...
	/*}else{
	  $con_cbt  = "Select *
	              From contab_trans_cli where CONTA_TRS_EMP = $nitcli and CONTA_TRS_NRO = $nro_doc  
				  and CONTA_TRS_USR_BAJA is null 
				  order by 'CONTA_TRS_CTA'";			  
	} */			  
...
<form name="form2" method="post" action="con_retro_1.php" onSubmit="return">
	...
	<table width="85%"  border="1" cellspacing="1" cellpadding="1" align="center" class="table_usuario">
		<?php
	 	...
	 	?>
  		Nueva Fecha
  		 <input class="txt_campo" type="text" id="datepicker" name="fec_nue" maxlength="10"  size="10" value="<?=$fecha;?>"> 
<br>
 <?php
}
	 ?>
...
// if( $_SESSION['cargo'] <> 4){		
	...
	?>
	 ...
	 <tr>
			  ...
			  <td align="left"><?php echo utf8_encode($desc); ?>
			  <br><?php echo utf8_encode($glosa_ind); ?> </td>
...
<br><input class="btn_form" type="submit" name="accion" value="Salir Consulta">
<?php }?>
