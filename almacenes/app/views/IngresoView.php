    ...
    */
    ...
    public function setMigaPan($template){
        $enlaces = array('modulo' => array('enlace' =>'../menu_s.php', 'imagen'=>'../img/app folder_32x32.png', 'titulo' => 'MODULOS'),
                         'ventas' => array('enlace' =>'../modulo.php?modulo=40000', 'imagen'=>'../img/order_32x32.png', 'titulo' => 'ALMACENES'),
                         'gestion' => array('enlace' =>'#', 'imagen'=>'../img/ventas_32x32.png', 'titulo' => 'GEST. ALMACENES')); 
    ...
    public function runIndex(){
        $template = file_get_contents('app/site/ingreso.html');
        $template = str_replace('{titulo_modulo}', 'INGRESO DE ALMANCENES', $template);
        $template = $this->configuracionHead($template);
        $header = file_get_contents('app/site/header.html');
        $header = $this->settingHeaders($header);
        $template = str_replace('{clasejs}', 'app/site/js/Ingreso.js', $template);
        ...
        $content_cab_venta = file_get_contents('app/site/ingreso_tpl/cabecera_ingreso_tpl.html');
        $content_det_venta = file_get_contents('app/site/ingreso_tpl/detalle_ingreso_tpl.html');
        $cliente_venta_tpl = file_get_contents('app/site/ingreso_tpl/proveedor_ingreso_tpl.html');
