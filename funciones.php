...
$con_trc = "SELECT sum(CAJA_DEP_IMPO) FROM caja_deposito where CAJA_DEP_USR_ALTA = '$log_usr'
           ...
           substr(CAJA_DEP_CTA_CTB,1,4)='1111' and CAJA_DEP_USR_BAJA is null ";
$res_trc = mysql_query($con_trc);
  while ($lin_trc = mysql_fetch_array($res_trc)) {
...
?>
<?php
/**  Esta parte es la para la facturacion  ***/
...
?>
<?php
function verif_cierre_cja($fec_ult,$log_usr) 
{
$mone = 1;
if ($mone == 1){
...
?>
<?php
function leer_desc_propios($grup,$cod) 
{
$desc = "";
   $consulta  = "Select GRAL_PAR_PRO_DESC From gral_param_propios where GRAL_PAR_PRO_GRP = $grup and GRAL_PAR_PRO_COD = $cod";
   $resultado = mysql_query($consulta)or die('No pudo seleccionarse gral_param_propios')  ;		  
		while ($linea = mysql_fetch_array($resultado)) { 
		      $desc = $linea['GRAL_PAR_PRO_DESC'];
			 
	       	   }	  
 return $desc;
 }		  
?>
<?php //Funciones para personal  ?> 
<?php
function leer_nro_empleado() 
{
$consulta  = "SELECT PER_MAE_NUMERICO FROM per_maestro where PER_MAE_USR_BAJA is null ORDER BY PER_MAE_NUMERICO DESC LIMIT 0,1";
$resultado = mysql_query($consulta)or die('No pudo seleccionarse tabla');
$linea = mysql_fetch_array($resultado);
$nro_emple = 0;
$nro_emple = $linea['PER_MAE_NUMERICO'];
if ($nro_emple == 0) {
   	$nro_emple = 1;
	} else {
	$nro_emple = $linea['PER_MAE_NUMERICO'] + 1;
	}
   return $nro_emple; 
}
?> 
<?php
function leer_nro_pertran() 
{
	$consulta  = "SELECT PER_TRA_SEC FROM per_trans where PER_TRA_USR_BAJA is null ORDER BY PER_TRA_SEC DESC LIMIT 0,1";
$resultado = mysql_query($consulta)or die('No pudo seleccionarse tabla');
$linea = mysql_fetch_array($resultado);
$nro_solic = 0;

$nro_solic = $linea['PER_TRA_SEC'];
//echo $nro_solic;
if ($nro_solic == 0) {
   	$nro_solic = 1;
	} else {
	$nro_solic = $linea['PER_TRA_SEC'] + 1;
	}
   return $nro_solic; 
}
?> 	
<?php
function leer_nro_pervac() 
{
	$consulta  = "SELECT PER_VAC_TRAN FROM per_vacacion where PER_VAC_USR_BAJA is null ORDER BY PER_VAC_TRAN DESC LIMIT 0,1";
$resultado = mysql_query($consulta)or die('No pudo seleccionarse tabla');
$linea = mysql_fetch_array($resultado);
$nro_solic = 0;

$nro_solic = $linea['PER_VAC_TRAN'];
//echo $nro_solic;
if ($nro_solic == 0) {
   	$nro_solic = $nro_solic + 1;
	} else {
	$nro_solic = $linea['PER_VAC_TRAN'] + 1;
	}
   return $nro_solic; 
}
?> 	
...
$ult_dia = strftime("%d", mktime(0, 0, 0, $mes+1, 0, $ano)); 

//$ult_dia =  ultimo_dia(2,2008); 
   return $ult_dia; 
}
?> 	
<?php
function leer_cta_per($tip,$mon) 
{
//echo "llega"."tip".$tip."top".$top."est".$est."mon".$mon;
 $con_cartc = "Select PER_VIA_CTA_CTB,PER_VIA_CTA_COD From per_via_cta where PER_VIA_CTA_GRP = $tip  and PER_VIA_MON = $mon and  
               PER_VIA_CTA_USR_BAJA is null ";
 $res_cartc = mysql_query($con_cartc)or die('No pudo seleccionarse tabla caja_transac 1')  ;
	while ($lin_cartc = mysql_fetch_array($res_cartc)) {
	       $cta_cart = $lin_cartc['PER_VIA_CTA_CTB']; 
		   $cta_tip = $lin_cartc['PER_VIA_CTA_COD']; 	
	      }
//echo $cta_cart, "cta cart";		  
 return $cta_cart;
 }		  
...
<?php //Funciones para ventas   ?> 
<?php
/** Esta tambien es parte de la facturacion +*********************** FACTURA *****************************/
function leer_nro_co_ven() 
{
...
$resultado = mysql_query($consulta);
$linea = mysql_fetch_array($resultado);
$nro_tran = $linea['caja_ventas_corr'];
...
?>
<?php
function leer_fechr_pro_ve($nrotra) 
{
$consulta  = "SELECT caja_ventas_fch_hra_alta FROM caja_ventas where caja_ventas_corr = $nrotra
               ORDER BY caja_ventas_fch_hra_alta
			  DESC LIMIT 0,1";
$resultado = mysql_query($consulta);
$linea = mysql_fetch_array($resultado);
$nro_tran = $linea['caja_ventas_fch_hra_alta'];
return $nro_tran; 
}
?>
