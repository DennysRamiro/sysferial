...
class Cliente{
    ...
    public function listaClientes($start, $limit){
    	$query = 'SELECT * 
                  FROM vent_cliente 
                  WHERE vent_cli_usr_baja IS NULL
                  order by vent_cli_id desc
                  LIMIT '.$start.',10';
      //print_r($query);
    	return $this->mysql->query($query);
    }
    ...
    public function guardarCliente($datos){
      ...
      $valores['vent_cli_fch_nac'] = $util->cambiaf_a_mysql($datos['txt_vent_cliente_fecha_nac']);
    ...
    public function guardarClienteBot($datos){
      ...
      $valores['vent_cli_fch_nac']=$util->cambiaf_a_mysql($datos['txt_vent_cliente_fecha_nac_bot']);
     ...
     **/
    public function getCodigoCliente($inicial, $num_cel){
      $codigo = $inicial;
