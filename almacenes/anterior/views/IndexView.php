  ...
  require_once "models/IndexModel.php";
    ...
    public function setNuevoFormIngreso($template){
        ...
        $template = str_replace('{destino_edit}', $dest_edit, $template);
          ...
          $tipoIngreso = $index_model->getTipoIngreso();

            $tipo_ing = '<select name="ingr_tipo_ingreso" size="1" size="10" id="ingr_tipo_ingreso" style="height:30px">';
            $cont=0;
            foreach($tipoIngreso as $key => $tipoIngreso){
                $tipo_ing = $tipo_ing.'<option value="'.$tipoIngreso['gral_param_propios']['GRAL_PAR_PRO_ID'].'" >'.$tipoIngreso['gral_param_propios']['GRAL_PAR_PRO_DESC'].'</option>';
            }
            $tipo_ing = $tipo_ing.'</select>';
            $template = str_replace('{tipo_ingreso}', $tipo_ing, $template);
