...
function Cliente(){
    ...
    this.buscarClienteXPalabraCampo = function(){
        $.getJSON('?action=clientes&tp=getDataClientexId', {client_id:$('#id_unico_cliente_vent').val(), start : 0,limit : 10}, function(res){
            ...
            tabla_cabecera = tabla_cabecera+'<tr><th>N</th><th>CODIGO</th><th>NOMBRE</th><th>APELLIDO</th><th>DIRECCION</th><th>TELEFONO</th><th>CELULAR</th><th>EMAIL</th><th>EDITAR</th><th>ELIMINAR</th>';
    ...
    this.nuevoCliente = function (){
        ...
        $('#txt_vent_cliente_fecha_nac').datepicker({
            showOtherMonths: true,
            selectOtherMonths: true,
            dateFormat:'dd/mm/yy'
        });
        util.validarCampo('txt_vent_cliente_fecha_nac', 'div_error_vent_error_cliente_fecha_nac', 'Es necesario seleccionar una fecha de nacimiento');
    ...
    this.resetForm = function(){
        $('#txt_vent_cliente_fecha_nac').val("");
    ...
    this.enviarFormularioNuevoCliente = function (){
        ...
        $('#form_vent_nuevo_cliente').submit(function(evt){
            ...
            }else if($.trim($('#txt_vent_correo_cliente').val()) == ""){
            ...
            }else if($.trim($('#txt_vent_direccion_cliente').val()) == ""){
    ...
    this.editarDatosCliente = function (){
        $('#form_vent_edit_cliente').submit(function(evt){
            if($.trim($('#txt_vent_nombre_cliente_edit').val()) == ""){
                ...
                util.mostrarMensajeAlerta(0, 'El Correo no puede estar vacio', 'Advertencia');
            }else if($.trim($('#txt_vent_cliente_fecha_nac_edit').val()) == ""){
                util.mostrarMensajeAlerta(0, 'El Fecha Nacimiento no puede estar vacio', 'Advertencia');
                ...
                console.log(datos);
                $.ajax({
                    url:'index.php?action=clientes&tp=saveEditCliente',
                    data:datos,
                    type:'GET',
                    dataType:'json',
                    success:function(res){
    ...
    this.editarCliente = function(id_cliente){
        $('#form_vent_edit_cliente')[0].reset();
        $.getJSON('?action=clientes&tp=getDataCliente', {client_id:id_cliente}, function(res){
            ...
            $('#vent_cli_nit_empresa_edit').val(res.vent_cli_nit_empresa);
            $('#vent_cli_telf_empresa_edit').val(res.vent_cli_telf_empresa);
            $('#vent_cli_fax_empresa_edit').val(res.vent_cli_fax_empresa);
            $('#vent_cli_correo_edit').val(res.vent_cli_correo);
            $('#id_unico_cliente').val(res.vent_cli_cod_unico);
            $('#codigo_cliente').val(res.vent_cli_codigo_cliente);
        });
        $('#txt_vent_cliente_fecha_nac_edit').datepicker({
            showOtherMonths: true,
            selectOtherMonths: true,
            dateFormat:'dd/mm/yy'
        });
    ...
    this.cambiarFormatoFechaNormal = function(fecha_mysql){
        var fechaArray = fecha_mysql.split('-');
        var nuevaFecha = fechaArray[2]+"/"+fechaArray[1]+"/"+fechaArray[0];
        return nuevaFecha;
    }
    /**
