	...
	this.listarIngresosXConfirmar = function(start, limit, num){
		...
		$.ajax({
			...
			success: function(resul){
				...
				paginacion_div = '<div id="paginacion"> <div id="titulo_paginacion"></div><div id="paginacion_tabla">'
				paginacion_div = '<div id="paginacion"><div id="titulo_paginacion"></div><div id="paginacion_tabla">'
				for (var i=1;i<=paginas;i++) {
					paginacion_div +='<div id="'+i+'" class="pagina">';
					if(pag == i){
						paginacion_div +=i;
					}else{
						paginacion_div +='<a href="#" onClick="new Index().listarIngresosXConfirmar('+i+', '+limit+', '+num+')">'+i+'</a>';						
					}
					paginacion_div +='</div>';
				}
				paginacion_div +='</div></div>';
		...
		$.ajax({
			...
			success: function(resul){
				...
				var tabla_cabecera_ingresos = '<div id="buscar_not_ing"><span><strong>BUSCAR :</strong></span> <input type="text" name="palabra_ing_buscar" id="palabra_ing_buscar" class="txt_campo2"><input type="button" value="Buscar" class="btn_form" onclick="new Index().buscarNotaIngBoton(0,'+limit_par+','+num_par+');"><input type="hidden" name="id_unico_not_ing" id="id_unico_not_ing">';
	          		...
	          		tabla_cabecera_ingresos = tabla_cabecera_ingresos+'<tr><th>NRO</th><th>CODIGO</th><th>NOMBRE</th><!--th>MOTIVO</th--><th>ORIGEN</th><th>DESTINO</th><th>FECHA</th><th>ESTADO</th><th>EDITAR</th><th>CANCELAR</th><th>NOTA INGRESO</th></tr>';
	            ...
	            if(resul.completo){
	            	var total=resul.total;
		              ...
		              if(ingresos.alm_cab_ing_egr_motivo_registro == 1){
		              	tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos +'<td align="center"><a href="#" onclick="new Index().eliminarNotaEntrega(\''+ingresos.alm_cab_ing_nro_unico+'\')"><img src="../img/delete_32x32.png" align="absmiddle"><br>Cancelar</a></td>';
		              }else if(ingresos.alm_cab_ing_egr_motivo_registro == 2){
		              	tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos +'<td align="center">INGRESADO</td>';
		              }
		              tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos +'<td align="center"><a href="#" onclick="new Index().notaIngreso(\''+ingresos.alm_cab_ing_nro_unico+'\')"><img src="../img/nota_editar_alm_32x32.png" align="absmiddle"><br> Nota Ingreso</a></td>';
		          	 
		         	});
		            tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos+'</tr></table>';
      ...
      var id = ui.item.id;
          $.ajax({
            ...
            success: function(resultado){
                ...
                var tabla_cabecera_ingresos = '<div id="buscar_not_ing"><span><strong>BUSCAR:</strong></span> <input type="text" name="palabra_ing_buscar" id="palabra_ing_buscar" class="txt_campo2"><input type="button" value="Buscar" class="btn_form" onclick="new Index().buscarNotaIngBoton(0,'+limit_par+','+num_par+');"><input type="hidden" name="id_unico_not_ing" id="id_unico_not_ing">';
          	        tabla_cabecera_ingresos = tabla_cabecera_ingresos+'<table id="tabla_ingreso_almacenes" class="table_usuario">';
          		    tabla_cabecera_ingresos = tabla_cabecera_ingresos+'<tr><th>NRO</th><th>CODIGO</th><th>NOMBRE</th><th>ORIGEN</th><th>DESTINO</th><th>FECHA</th><th>EDITAR</th><th>CANCELAR</th><th>NOTA INGRESO</th></tr>';
 		        $('#div_lista_ingreso_almacenes').append(tabla_cabecera_ingresos);
                var tabla_cabecera_ingresos_datos = '<tr><td>1</td>';
	                ...
	                if(resultado[0].alm_cab_ing_egr_motivo_registro == 1){
		              	tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos +'<td align="center"><a href="#" onclick="new Index().eliminarNotaEntrega(\''+resultado[0].alm_cab_ing_nro_unico+'\')"><img src="../img/delete_32x32.png" align="absmiddle"><br>Cancelar</a></td>';
		              }else if(resultado[0].alm_cab_ing_egr_motivo_registro == 2){
		              	tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos +'<td align="center">INGRESADO</td>';
		              }
		              tabla_cabecera_ingresos_datos = tabla_cabecera_ingresos_datos +'<td align="center"><a href="#" onclick="new Index().notaIngreso(\''+resultado[0].alm_cab_ing_nro_unico+'\')"><img src="../img/nota_editar_alm_32x32.png" align="absmiddle"><br> Nota Ingreso</a></td>';
   ...
   */
	this.grabarNuevoIngreso = function(){  
		...
		$('#form_alm_nuevo_ingreso').submit(function(evt){
			...
			}else if ($('#alm_ing_descr').val() == "") {
				util.mostrarMensajeAlerta(0,'La <b>Descripcion</b> es Obligatoria','Advertencia');
			}else if ($('#alm_fec_ing').val() > fech) {
				util.mostrarMensajeAlerta(0,'La <b>Fecha</b> no puede ser mayor ','Advertencia');
			}else{
				util.startPreloader();

				datos_ing = $(this).serialize();
				//console.log(datos);
				$.ajax({
...
this.notaIngreso = function(codigo_unico){
	$(location).attr('href','site/ingreso_tpl/nuevo_form_reporte_ingreso_tpl.php?val='+codigo_unico);
}
	...
	this.IngresaNuevoItemCabDet = function (codigo_unico){
		...
		$.getJSON('?accion=ingreso&tp=getDatosIngresoDet',{unico:codigo_unico},function(res){
			...
			var ing_nota = ing_nota + '<tr>';
				...
				ing_nota = ing_nota +'<th>CANTIDAD</th> <th>EDITAR</th><th>CANCELAR</th></tr>';
			...
			if (res.det!=0) {
					$.each(res, function(index, ingresos){
						    ...
						    mod_det_datos = mod_det_datos +'<td>'+ingresos.alm_ing_egr_det_cantidad+' '+ingresos.alm_ing_egr_det_unidad+'</td>';
